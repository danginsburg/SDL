...
static VkResult VULKAN_CreateDeviceResources(SDL_Renderer *renderer)
{
    ...
    for (uint32_t i = 0; i < NUM_SHADERS; i++) {
        ...
        /* YUV / NV12 */
        else if (i >= SHADER_YUV_JPEG && i <= SHADER_NV_BT709) {
            layoutBindings[0].binding = 0;
            layoutBindings[0].descriptorCount = 1;
            layoutBindings[0].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLER;
            layoutBindings[0].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
            ...
            /* YUV has 3 textures and one sampler (4 total), NV12/NV21 has 2 textures and one sampler (3 total) */
            descriptorSetLayoutCreateInfo.bindingCount = (i >= SHADER_YUV_JPEG && i <= SHADER_YUV_BT709) ? 4 : 3;
            descriptorSetLayoutCreateInfo.pBindings = layoutBindings;
        }
...
static int VULKAN_CreateTexture(SDL_Renderer *renderer, SDL_Texture *texture, SDL_PropertiesID create_props)
{
    ...
    SDL_SetProperty(SDL_GetTextureProperties(texture), SDL_PROP_TEXTURE_VULKAN_TEXTURE_POINTER, (void*) textureData->mainImage.image);
    ...
    if (texture->format == SDL_PIXELFORMAT_YV12 ||
        ...
        SDL_SetProperty(SDL_GetTextureProperties(texture), SDL_PROP_TEXTURE_VULKAN_TEXTURE_U_POINTER, ( void* ) textureData->mainImageU.image);
        ...
        SDL_SetProperty(SDL_GetTextureProperties(texture), SDL_PROP_TEXTURE_VULKAN_TEXTURE_V_POINTER, ( void* ) textureData->mainImageV.image);
    }
        ...
        VkComponentMapping swizzle = rendererData->identitySwizzle;
        // Use imageView swizzle to swap U <-> V so we can use one shader for both NV12/NV21
        if (texture->format == SDL_PIXELFORMAT_NV21) {
            swizzle.r = VK_COMPONENT_SWIZZLE_G;
            swizzle.g = VK_COMPONENT_SWIZZLE_R;
        }
        /* Allocate interleaved UV plane as R8G8 */
        result = VULKAN_AllocateImage(rendererData, width, height, VK_FORMAT_R8G8_UNORM, usage, swizzle, VK_NULL_HANDLE, &textureData->mainImageUV);
        if (result != VK_SUCCESS) {
            VULKAN_DestroyTexture(renderer, texture);
            SDL_LogError(SDL_LOG_CATEGORY_RENDER, "VULKAN_AllocateImage(): %s\n", SDL_Vulkan_GetResultString(result));
            return result;
...
static SDL_bool VULKAN_SetCopyState(SDL_Renderer *renderer, const SDL_RenderCommand *cmd, const Float4X4 *matrix, VULKAN_DrawStateCache *stateCache)
{
    ...
    if (!VULKAN_SetDrawState(renderer, cmd, SHADER_RGB, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 1, &textureData->mainImage.imageView,
    ...
    if (textureData->yuv) {
        ...
        case SDL_YUV_CONVERSION_JPEG:
            ...
            break;
        case SDL_YUV_CONVERSION_BT601:
            shader = SHADER_NV_BT601;
            break;
        case SDL_YUV_CONVERSION_BT709:
            shader = SHADER_NV_BT709;
            break;
        default:
            return SDL_SetError("Unsupported YUV conversion mode");
        }
